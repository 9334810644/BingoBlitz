...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200/60 sticky top-0 z-30 px-3 sm:px-6 py-3 text-slate-800 shadow-sm transition-all">
      <div className="max-w-6xl mx-auto flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="hidden sm:inline-block text-[10px] font-black uppercase tracking-wider text-indigo-700 bg-gradient-to-r from-indigo-50 to-purple-50 px-2.5 py-0.5 rounded-full border border-indigo-200/80 shadow-xs">
              5-Line Cut
            </span>
          </div>
        </div>
        ...
        {gameMode === 'multiplayer' && roomCode ? (
          <div className="flex items-center gap-2 bg-gradient-to-r from-indigo-50 to-purple-50 px-3.5 py-1.5 rounded-full border border-indigo-200/80 text-xs font-bold text-indigo-950 shadow-xs">
          ...
          <div className="hidden md:flex items-center gap-2 bg-slate-100/80 backdrop-blur-md px-4 py-1.5 rounded-full border border-slate-200/80 text-xs font-extrabold text-slate-700 shadow-xs">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            className={`p-2 rounded-xl border transition-all active:scale-95 cursor-pointer ${
              soundSettings.soundEnabled
                ? 'bg-indigo-50 text-indigo-700 border-indigo-200/80 shadow-xs hover:bg-indigo-100'
                : 'bg-slate-100/90 text-slate-400 border-slate-200 hover:bg-slate-200/70 hover:text-slate-800'
            }`}
          ...
          <button
            onClick={onOpenStats}
            className="p-2 sm:px-3 sm:py-2 bg-slate-100/90 hover:bg-slate-200/70 text-slate-700 border border-slate-200 rounded-xl transition-all active:scale-95 cursor-pointer flex items-center gap-1.5 text-xs font-bold shadow-xs"
            title="Statistics"
          >
